 ...
 *
 * 使用：
 * toActivity或startActivityForResult (PlacePickerWindow.createIntent(...), requestCode);
 * 然后在onActivityResult方法内
 * data.getStringExtra(PlacePickerWindow.RESULT_PLACE);           可得到地址拼接字符串
 * date.getStringArrayExtra(PlacePickerWindow.RESULT_PLACE_LIST); 可得地址列表
 */
public class PlacePickerWindow extends BaseViewBottomWindow<List<Entry<Integer, String>>, GridPickerView> {
    ...
    public void initData() {//必须调用
        ...
        maxLevel = getIntent().getIntExtra(INTENT_MAX_LEVEL, 1);
        ...
        runThread(TAG + "initData", new Runnable() {
            ...
            public void run() {
                ...
                if (maxLevel < 1) {
                    configList.add(new GridPickerConfig("", "北京", 0));
                }
                if (maxLevel == 1) {
                    configList.add(new GridPickerConfig("", "东城", 0));
                }
                ...
                list = getList(0, selectedItemNameList);
                runUiThread(new Runnable() {
    ...
    protected void setResult() {
        ...
        intent.putStringArrayListExtra(RESULT_PLACE_LIST, placeList);
